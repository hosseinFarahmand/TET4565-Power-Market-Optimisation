...
while upper_bound - lower_bound > tolerance:
    ...
    # Update lower bound
    lower_bound = max(lower_bound, master.Objective())
    ...
    # Update upper bound
    upper_bound = min(upper_bound, sub.Objective())
    ...
    if iteration >= 1:
        def benders_cut_rule(master):
            return (
                sum(sub.dual[sub.Puu[k, s]] * (master.Ru[k] - sub.Ru_fixed[k]) for k in sub.k for s in sub.s if sub.Puu[k, s] in sub.dual) +
                sum(sub.dual[sub.Pdu[k, s]] * (master.Rd[k] - sub.Rd_fixed[k]) for k in sub.k for s in sub.s if sub.Pdu[k, s] in sub.dual) +
